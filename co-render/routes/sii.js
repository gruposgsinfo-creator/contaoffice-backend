...
async function getOrInstallChromium() {
  ...
  if (process.env.CHROMIUM_PATH && existsSync(process.env.CHROMIUM_PATH)) {
                _chromiumPath = process.env.CHROMIUM_PATH;
                return _chromiumPath;
  }

  // 2. Paths comunes Linux/Debian
  const systemPaths = [
  ...
  // 3. Buscar con which (incluye paths de Nix en $PATH)
  const cmds = ['chromium', 'chromium-browser', 'google-chrome'];
  ...
  // 4. Buscar en el store de Nix
  try {
                const p = execSync(`find /nix/store -name chromium -type f 2>/dev/null | head -1`, {
                                encoding: 'utf8',
                                timeout: 10000
                }).trim();
                if (p && existsSync(p)) { _chromiumPath = p; return p; }
  ...
  // 5. Instalar via apt-get como fallback
  try {
                console.log('Instalando chromium via apt-get...');
                execSync('apt-get update -y && apt-get install -y chromium-browser --no-install-recommends', {
                ...
                console.log('apt-get falló:', e.message.substring(0, 200));
  }

  throw new Error('No se pudo encontrar Chromium en el sistema.');
}
...
router.post('/consultar', protect, async (req, res) => {
            ...
            let browser;
              ...
              const urlActual = page.url();
                            if (urlActual.includes('InicioAutenticacion') || urlActual.includes('error')) {
                                            await browser.close();
              ...
              // F29
              try {
                              await page.goto('https://www4.sii.cl/consdcxInernet/index.html', {
                                                waitUntil: 'networkidle2', timeout: 30000
                              });
              ...
              // F22 (Renta)
              try {
                              await page.goto('https://www4.sii.cl/consdcxInernet/index.html?type=F22', {
                                                waitUntil: 'networkidle2', timeout: 30000
                              });
                              await page.waitForSelector('table', { timeout: 10000 });
              ...
              try {
                              await page.goto('https://www4.sii.cl/djindexInternetui/index.html', {
                                                waitUntil: 'networkidle2', timeout: 30000
                              });
                              await page.waitForSelector('table', { timeout: 10000 });
